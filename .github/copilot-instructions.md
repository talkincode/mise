# misec Copilot Instructions

misec 是一个**命令行上下文准备工具**，将项目中的文件、片段和锚点整理成 Agent 可直接消费的结构化输出。**它只负责读取和组织材料，不生成答案。**

> **重要**: misec 安装在 `~/bin/misec`，所有命令通过 `run_in_terminal` 执行。
...
# 3. 了解项目结构
misec scan --type file --max-depth 2 --pretty

# 4. 查看已标记的锚点
misec anchor list --pretty
```
...
| 场景     | 原因                 | 应使用                   |
| -------- | -------------------- | ------------------------ |
| 编辑文件 | misec 只读取         | `replace_string_in_file` |
| 运行命令 | misec 不执行任意命令 | `run_in_terminal`        |
| 创建文件 | misec 不创建文件     | `create_file`            |
...
# 探索
misec scan --type file --max-depth 2 --pretty
misec find "readme"                       # 按名称查找
misec match "TODO|FIXME" --pretty         # 搜索模式
misec ast "fn main" src/                  # AST 搜索

# 提取
misec extract src/main.rs --lines 1:100   # 提取行范围

# 锚点
misec anchor list --brief --pretty        # 列出锚点概览（省Token，推荐初次探索）
misec anchor list --pretty                # 列出锚点（含完整内容）
misec anchor list --tag core              # 按标签过滤
misec anchor get intro --with-neighbors 3 # 获取锚点+上下文
misec anchor lint                         # 检查问题
misec anchor mark src/cli.rs --start 10 --end 50 --id cli.commands --tags core
misec anchor unmark src/cli.rs --id cli.commands

# 依赖
misec deps src/cli.rs                     # 正向依赖
misec deps src/cli.rs --reverse           # 反向依赖
misec deps --deps-format tree             # 树形视图

# 变更
misec impact                              # 未暂存变更
misec impact --staged                     # 已暂存变更
misec impact --impact-format summary      # 人类可读

# 工作流
misec flow stats --stats-format summary   # 项目统计
misec flow outline --outline-format markdown
misec flow pack --anchors a,b --max-tokens 4000
```
...
```bash
misec scan --type file --max-depth 3 --pretty   # 结构
misec match "fn main|async fn" src/ --pretty    # 找入口
misec deps src/main.rs --deps-format tree       # 依赖
```

### PR 审查

```bash
misec impact --staged --impact-format summary   # 变更影响
misec deps changed_file.rs --reverse            # 谁依赖它
```

### 上下文打包

```bash
misec flow pack --anchors core,cli --max-tokens 4000
