...
run_cmd "$MISE deps src/cli.rs --root . --deps-format tree"
run_cmd "$MISE deps src/cli.rs --root . --reverse --deps-format tree"
run_cmd "$MISE deps --root . --deps-format dot | head -20"
run_cmd "$MISE deps --root . --deps-format mermaid | head -20"
run_cmd "$MISE deps --root . --deps-format table | head -15"

echo -e "\n${GREEN}【9. impact - 变更影响分析】${NC}\n"
run_cmd "$MISE impact --root . --impact-format summary"
run_cmd "$MISE impact --root . --impact-format table"
run_cmd "$MISE impact --root . --impact-format jsonl"
run_cmd "$MISE impact --staged --root . --impact-format summary"
